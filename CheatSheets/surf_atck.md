...
8. [Интеграция с нормативами ФСТЭК и ИСП РАН](#8-интеграция-с-нормативами-фстэк-и-исп-ран)
...
- подготовку ПО к сертификации по требованиям ФСТЭК РФ и ИСП РАН;
...
- практик ИСП РАН в части моделей угроз и технических профилей защищённости.
...
## 8. Интеграция с нормативами ФСТЭК и ИСП РАН

Анализ поверхности атаки и построение карты фаззинга напрямую соотносятся с требованиями нормативных документов ФСТЭК России и методических рекомендаций ИСП РАН. Корректное выполнение этих процедур обеспечивает соответствие требованиям по контролю защищённости и обоснование охвата фаззинг-тестированием при сертификации.
...
### 8.2 Практики ИСП РАН

| Подход / метод | Применение в ИСП РАН |
|----------------|----------------------|
| **Обоснование полноты тестирования** | Через карту фаззинга фиксируются все охваченные интерфейсы, типы входов, границы доверия. Используется как артефакт при подаче на сертификацию. |
| **Статический и динамический анализ до фаззинга** | Является обязательным предварительным этапом. Описывается в разделе подготовки к fuzzing (в отчёте). |
| **Документирование связей вход → функция → результат** | Необходима формализация трассировки: какой вход активирует какую функцию и к какому эффекту приводит. В ИСП РАН это используется для моделирования детерминированных цепочек передачи данных. |
| **Подтверждение результативности инструментов** | При фаззинге на основе карты указываются используемые инструменты и методы оценки их покрытия (coverage-based fuzzing, directed fuzzing, positive-negative corpus). |
| **Интеграция с профилем защиты** | Информация из анализа ПА используется для дополнения и корректировки Профиля защиты ПО, особенно разделов, касающихся архитектуры, зон доверия и интерфейсов. |

---

### 8.3 Сопоставление шагов методики и требований ФСТЭК
...
Результаты анализа поверхности атаки, построения карты фаззинга и последующего тестирования должны быть оформлены в виде технической документации, соответствующей внутренним регламентам организации, требованиям регуляторов (ФСТЭК, ИСП РАН), а также потребностям команд разработки, QA и ИБ.
...
| **Акт анализа уязвимостей** | Документ, входящий в пакет сертификации (ФСТЭК/ИСП РАН), включает выводы по результатам анализа ПА и выявленных уязвимостей. |
