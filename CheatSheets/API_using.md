...
### **Важно запомнить:**
- `Content-Type` важен для `POST`/`PUT`. Для JSON это `application/json`.
- `jq` — ваш лучший друг для работы с JSON.
- Всегда проверяйте статус-коды HTTP ответа (`-i` или `-v`).
- Для отладки используйте `-v` (verbose) для подробного вывода.

### **Типичные ошибки:**
- **401 Unauthorized / 403 Forbidden**: Проблемы с аутентификацией/авторизацией. Проверьте токены, API-ключи, логин/пароль.
- **404 Not Found**: Неправильный URL или ресурс не существует.
- **5xx Server Error**: Проблема на стороне сервера. Попробуйте позже или свяжитесь с поддержкой API.
...
## **Важно запомнить:**
- SOAP-запросы — это XML внутри XML (SOAP Envelope).
- Пространства имен (namespaces) в XML критически важны.
- `SOAPAction` (для SOAP 1.1) должен соответствовать операции в WSDL.
- Для SOAP 1.2 используется `Content-Type: application/soap+xml; charset=utf-8; action="<action_url>"`.

## **Типичные ошибки (общие):**
- Неправильно сформированный SOAP Envelope (синтаксис XML, неймспейсы).
- Ошибки в `SOAPAction` или его отсутствие.
- Проблемы с аутентификацией (SOAP может использовать WS-Security, HTTP Basic Auth и др., что усложняет `curl` запросы без доп. обработки).
...
## **Важно запомнить:**
- Всегда один endpoint для всех операций (обычно `/graphql`).
- Запросы всегда `POST` с `Content-Type: application/json`.
- Тело запроса: `{"query": "...", "variables": {...}}`.
- Ответ содержит поле `data` с результатами и/или поле `errors` с ошибками.

## **Типичные ошибки (общие):**
- Ошибки синтаксиса в GraphQL-запросе (забыли фигурные скобки, неверные имена полей).
- Запрос невалидных полей (не существуют в схеме).
- Проблемы с переменными (не тот тип, не переданы обязательные).
- JSON в `-d` невалиден (часто из-за проблем с кавычками и экранированием).
...
## **Важно запомнить"**
- gRPC использует HTTP/2 и Protobuf.
- `grpcurl` требует либо серверной рефлексии, либо `.proto` файлов.
- Данные для запросов (`-d`) передаются в JSON-формате, который `grpcurl` преобразует в Protobuf.
- По умолчанию используется TLS; для HTTP используйте `-plaintext`, для самоподписанных сертификатов `-insecure`.

## **Типичные ошибки (общие):**
- Проблемы с SSL/TLS сертификатами (используйте `-insecure` для тестов или `-cacert`).
- Серверная рефлексия отключена, и не предоставлены `.proto` файлы.
- Неправильный формат JSON для данных запроса (`-d`).
- Сетевые проблемы: фаервол, неправильный адрес/порт.
